...
- `./scripts/deploy.sh migrate` покрывает все миграции 001-009 (исправлено),
  но по-прежнему требует, чтобы контейнер `evo_postgres` уже был поднят.
- Миграция 009 (`evo_api_keys`, `evo_request_log`) добавлена для многоключевого
  личного кабинета и метрик скорости/токенов — см. `db/metrics.py`,
  `api/routes/me.py`, `api/routes/admin.py`.

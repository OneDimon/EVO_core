...
7. **Immune System** — `core/immune_system.py`, `core/sleep_mode.py`, `n8n/` —
   фоновые задачи (см. `SLEEP_MODE.md`), самовосстановление.

**Незакрытые пункты на момент этого аудита** (проверить актуальность перед
тем, как на них полагаться):
- n8n не подключён как сервис в `docker-compose.yml` (workflow есть в
  `n8n/evo_immune_system_workflow.json`, но не задеплоен).
- Миграция 009 не создана.
- `./scripts/deploy.sh migrate` не покрывает 004-008 на уже существующих БД.

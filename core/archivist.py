...
# ИСПРАВЛЕНО (после N12/P16): возврат к исходной нотации SCL_SYMBOLIC_NOTATION.md.
# Спецификация: "Φ — Базовый символ | Макро-область из 32 корней" — ОДНА греческая
# буква на корень, не латинский 2-буквенный код. Прошлый P16-фикс ошибочно ввёл
# латиницу (Tc, Mt...) — разошлось с LOCAL_MODEL_INSTRUCTIONS.md, SCL_FRACTAL_PROTOCOL.md
# и tests/test_notation.py, которые все написаны в терминах греческого алфавита.
#
# Используются только визуально однозначные символы: 23 строчных греческих буквы
# (без омографов латиницы там где это мешает) + 9 заглавных из тех что НЕ похожи
# на латиницу (Γ Δ Θ Λ Ξ Π Σ Φ Ψ — все визуально уникальны, в отличие от Α/Β/Ε/Κ/Ρ/Τ/Χ
# которые на письме неотличимы от A/B/E/K/P/T/X).
ROOT_CODES: dict[str, str] = {
    # Точные науки
    "Математика": "μ",    "Физика": "γ",       "Химия": "ζ",
    "Биология": "β",      "Информатика": "κ",   "Кибернетика": "ξ",
    # Инженерия
    "Технология": "τ",    "Архитектура": "λ",   "Механика": "ρ",
    "Электроника": "ε",   "Робототехника": "υ", "Автоматизация": "α",
    # ИИ и данные
    "ИИ": "φ",            "МашОбучение": "θ",   "Данные": "δ",
    "Алгоритмы": "ι",     "БазыДанных": "ν",    "Сети": "χ",
    # Философия и гуманитарные
    "Философия": "ω",     "Лингвистика": "η",   "Психология": "ψ",
    "Социология": "σ",    "Экономика": "π",     "Право": "Γ",
    # Практика
    "Медицина": "Δ",      "Педагогика": "Θ",    "Менеджмент": "Λ",
    "Маркетинг": "Ξ",     "Дизайн": "Π",        "Искусство": "Σ",
    # Системные
    "Безопасность": "Φ",  "Инфраструктура": "Ψ",
}

def _get_root_code(science: str) -> str:
    """
    Возвращает однобуквенный греческий символ макро-корня по SCL_SYMBOLIC_NOTATION.md.
    Ровно так, как задумано изначально: Φ^{a^m}_{s·n} — базовый символ = 1 буква.
    """
    if science in ROOT_CODES:
        return ROOT_CODES[science]
    # Fallback для неизвестного корня — не должен встречаться при валидных данных,
    # но не роняет запись: 'Ω' (последняя буква) как явный маркер "неизвестный корень".
    log.warning(f"[Archivist] Неизвестный макро-корень '{science}' — используется fallback Ω")
    return "Ω"
